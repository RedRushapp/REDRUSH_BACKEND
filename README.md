...
- `FRONTEND_URL` controls CORS access from the React app. Multiple URLs can be separated with commas.
...
During local development, other `localhost` and `127.0.0.1` ports are also accepted. In production, only `FRONTEND_URL` and the listed origins are accepted.
